...
# load in tau associated proteins
tau_file = r"C:\Users\qingh\OneDrive - The University of Sydney (Students)\VRIproteomics\Tau_proteins_analysis_merged.csv"

data_tau = pd.read_csv(tau_file)
...
tau_proteins = list(data_tau["Protein"])
# print(tau_proteins)
...
dep_flags["n_comparisons"] = dep_flags.sum(axis=1)
dep_flags["tau protein"] = list(i in tau_proteins for i in dep_flags.index)
dep_flags.to_csv("DEPs_overlap_fc_only_flags_tauproteins_labelled.csv")
# print(list(i in tau_proteins for i in dep_flags.index))